...
def classifiction_model_function(data, final_columns1, parameter_to_be_optimized, target):
    final_model1, data_setup1, best_model, configs_path, best_model_results, model_name = setting_up_automl_classification(data, final_columns1, parameter_to_be_optimized, target)
    X_train, y_train, X_test, y_test = save_feature_importance_classification(final_model1)
    retrain_data = data_set_classification(X_train, y_train, X_test, y_test)
    ...
    return train_set_columns, best_model, data_setup1, retrain_data, target_column, best_model_results, model_name, SE, predictions, X_train, y_train


def columns_for_model_building_classification(data):
    ...
    if target is not None:
        ...
        if all1:
            ...
            final_columns1 =  container1.multiselect("Select columns for model building:", columns, target)
            final_columns1.append(target)
...
def setting_up_automl_classification(data, final_columns, parameter_to_be_optimized, target):
    ...
    st.write('* Model Results After Tuning')
    st.write(f'* {parameter_to_be_optimized} value after optimization :', ((evaluated_model[parameter_to_be_optimized][0])*100).round(2))
...
def save_feature_importance_classification(final_model):
...
def data_set_classification(X_train, y_train, X_test, y_test):
    train_data = pd.concat([X_train, y_train], axis = 1)
    test_data = pd.concat([X_test,y_test], axis = 1)
    data = pd.concat([train_data, test_data], axis = 0)
    return data

def retrain_model_function_classification(retrain_data, final_columns2, parameter_to_be_optimized, target_column, best_model):
    final_model2, data_setup2 = retrain_model(retrain_data, final_columns2, parameter_to_be_optimized, target_column, best_model)
    X_train_new, y_train_new, X_test_new, y_test_new = save_feature_importance_classification(final_model2)
    SE, predictions = save_shap_file(final_model2, X_test_new)
    saving_top_features(SE, predictions, target_column)
    return final_model2

def retrain_model_classification(data, final_columns, parameter_to_be_optimized, target, best_model):
